...
export default function SignUp() {
  const [{email, password}, setFrom] = useState<LoginFormType>(initialFormState)
  const changed = useCallback(
    (key: string) => (e: ChangeEvent<HTMLInputElement>) => {
      setFrom(obj => ({...obj, [key]: e.target.value}))
    },
    []
  )
    ...
    <div className="flex flex-col min-h-screen border-gray-300 rounded-xl shadow-xl bg-gray-100 border">
      <div className="flex flex-col items-center justify-center flex-1 max-w-sm px-2 mx-auto">
        <div className="w-full px-6 py-8 text-black bg-white rounded shadow-md">
          ...
          <input
            type="text"
            className="w-full p-2 mb-4 input input-primary"
            name="eamil"
            placeholder="Eamil"
            value={email}
